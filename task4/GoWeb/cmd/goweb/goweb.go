...
import (
	...
	"GoStudy/task4/GoWeb/pkg/repository"
	"errors"
	"flag"
	"fmt"
	"log"
...
func main() {
	...
	for _, url := range webSites {
		prev_result, err := repository.SearchResult(url)
		if err != nil {
			log.Printf("GoWeb: %s", err)
		}
		if prev_result != nil {
			docs = append(docs, prev_result...)
			fmt.Println("GoWeb: Found in repository")
			continue
		}
